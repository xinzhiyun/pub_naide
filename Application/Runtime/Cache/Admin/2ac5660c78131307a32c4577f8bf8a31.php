...
<div class="content">
    ...
    <div class="row-fluid fl" id="main">
        <div class="tableBox">
            ...
            <form class="form-search" action="/index.php/Admin/Work/index" method="post">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    <?php if(!empty($list)): if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                        ...
                        <td><?php echo ($vo["pphone"]); ?></td>
                        <td><?php echo ($vo["type"]); ?></td>
                        <td><?php echo ($vo["content"]); ?></td>
                        <?php $address = $vo['province'].$vo['city'].$vo['district'].$vo['addess']; ?>
                        <td><?php echo ($vo['type'] === '维修' ? $vo['raddress']:$address); ?></td>
                        <td><?php echo ($vo["result"]); ?></td>
                        <td><?php if($vo['time']){ echo date('Y-m-d H:i:s',$vo['time']);} ?></td>
                        <td><?php echo (date('Y-m-d H:i:s',$vo["create_at"])); ?></td>
                        <td>
                            <?php switch($vo["result"]): case "未处理": ?><a class="handleBtn" ruleId="<?php echo ($vo["id"]); ?>">正在进行中</a><?php break;?>
                                <?php case "正在处理": ?><a href="/index.php/Admin/Work/edit/id/<?php echo ($vo["id"]); ?>/result/2">任务完成</a><?php break;?>
                ...
                $('.pagination ul span').wrap('<li class="active"></li>')
                 ...
                 /**************** 搜索关键字保留 -- 开始 ******************/
                    ...
                    $('button[type="reset"]').click(function(){
                        location.href = '<?php echo U("Admin/Work/index");?>';
                    ...
                    $(".handleBtn").click(function(){
                        ...
                        layui.use('layer', function(){
                            ...
                            layer.confirm('确定现在处理该条数据?', {icon: 3, title:'温馨提示'}, function(index){
                                window.location.href='<?php echo U('edit');?>?id='+id+'&result=1';
                                layer.close(index);
